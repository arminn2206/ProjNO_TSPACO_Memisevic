...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{3.6cm}>{\raggedright\arraybackslash}p{5.2cm}>{\raggedright\arraybackslash}p{6.2cm}@{}}
\toprule
...
The only substantive deviation in the problem being solved is the instance itself:
...
The proposal also listed the example's \code{Graph} structure among the parts to be adapted; Section~\ref{sec:b03} explains why it was removed instead.

\subsection{Starting Point: \texttt{B\_S03\_Maps}}\label{sec:b03}
...
\begin{algorithm}[tbp]
\caption{Ant System as implemented in \texttt{Model::runACO()}}\label{alg:as}
\begin{algorithmic}[1]
...
Algorithm~\ref{alg:as} combines the steps of Sections~2.3--2.7 into one complete run of \code{Model::runACO()}.
...
\begin{itemize}
  ...
  \item \sloppy Counters and flags crossing threads outside the lock (\code{_iteration},
...
\begin{enumerate}[label={[\arabic*]}]
  ...
  \item A. Memišević, \emph{Ant Colony Optimization for the Traveling Salesman Problem} -- project proposal,
        ETF Sarajevo (\code{Docs/NO_project_proposal_ArminMemisevic.pdf}).
